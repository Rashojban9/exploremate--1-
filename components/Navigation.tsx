...
export const Navbar = ({ onNavigate, isLoggedIn = false }: { onNavigate: (page: string) => void, isLoggedIn?: boolean }) => {
  ...
  const logoBg = scrolled ? 'bg-white/80 backdrop-blur-xl' : 'bg-white/60 backdrop-blur-md';
  ...
  return (
    <nav className={`fixed top-0 left-0 w-full z-[100] transition-all duration-500 px-4 md:px-0 ${scrolled ? 'bg-white/80 backdrop-blur-xl shadow-lg border-b border-slate-200/50 py-2.5 md:py-3' : 'bg-white/40 backdrop-blur-md py-5 md:py-6'}`}>
      <div className="max-w-7xl mx-auto md:px-8 flex items-center justify-between">
        <div className="flex items-center gap-3 cursor-pointer group" onClick={() => onNavigate('landing')}>
          <div className={`${logoBg} p-1.5 rounded-xl shadow-sm transition-colors duration-300 border border-white/10`}>
            <Logo className="w-7 h-7 md:w-8 md:h-8" variant="default" />
          </div>
          <span className={`font-display font-bold text-base sm:text-lg md:text-xl tracking-tight transition-colors duration-300 ${logoText}`}>ExploreMate</span>
        </div>

        {/* Desktop Menu */}
        <div className="hidden md:flex items-center gap-6 lg:gap-10">
          {menuItems.map((item) => (
             <button key={item.page} onClick={() => onNavigate(item.page)} className={`text-sm font-bold transition-colors tracking-wide ${textColor}`}>{item.label}</button>
          ))}
        ...
        <div className="hidden md:flex items-center gap-3 lg:gap-4">
          {isLoggedIn ? (
             <button onClick={() => onNavigate('dashboard')} className={`px-5 py-2.5 rounded-full text-sm font-bold transition-all shadow-lg shadow-sky-900/10 ${scrolled ? 'bg-sky-900 text-white hover:bg-sky-800' : 'bg-sky-900 text-white hover:bg-sky-800'}`}>
               Dashboard
             </button>
          ) : (
            <>
              ...
              <button onClick={() => onNavigate('signup')} className={`px-5 lg:px-6 py-2.5 rounded-full text-sm font-bold transition-all shadow-lg shadow-sky-900/10 bg-sky-900 text-white hover:bg-sky-800`}>
                Sign Up
              </button>
            </>
          )}
