...
import { createTrip, deleteTrip, listTrips, updateTrip, type ApiTrip, type ApiTripStatus, type TripResponse } from '../services/api';
...
const mapTripToUI = (trip: TripResponse): TripUI => {
    const now = new Date();
    now.setHours(0, 0, 0, 0);

    const startDate = trip.startDate && trip.startDate !== 'TBD' ? new Date(trip.startDate) : null;
    const endDate = trip.endDate && trip.endDate !== 'TBD' ? new Date(trip.endDate) : null;

    let baseStatus = statusFromApi(trip.status as ApiTripStatus);
    let daysLeft: number | null = null;
    let isMissed = false;
    let isOngoing = false;

    if (startDate && baseStatus === 'upcoming') {
        const tempStart = new Date(startDate);
        tempStart.setHours(0, 0, 0, 0);

        const diffTime = tempStart.getTime() - now.getTime();
    ...
    return {
        ...
        image: matchTripImage(trip.placeName, trip.placePhotos?.[0]),
        collaborators: [],
        daysLeft,
        isMissed,
        isOngoing
...
const TripCard: React.FC<{ trip: TripUI; onClick: () => void; onDelete: (id: string) => void; onEdit: (trip: TripUI) => void; onComplete: (id: string) => void }> = ({ trip, onClick, onDelete, onEdit, onComplete }) => {
    ...
    const onHover = (enter: boolean) => {
        ...
        if (enter) {
            ...
            const img = cardRef.current.querySelector('.trip-img');
            const action = cardRef.current.querySelector('.trip-action');
            if (img) gsap.to(img, { scale: 1.1, duration: 0.6 });
            if (action) gsap.to(action, { x: 0, opacity: 1, duration: 0.3 });
        } else {
            ...
            if (action) gsap.to(action, { x: -10, opacity: 0, duration: 0.3 });
        }
    };
            ...
            <div className="w-full md:w-72 h-56 md:h-auto rounded-[2rem] overflow-hidden relative shrink-0">
                ...
                <div className="absolute top-4 left-4 flex flex-col gap-2">
                    {trip.status === 'upcoming' && (
                        <>
                            {trip.isMissed ? (
                                <div className="bg-red-600/90 backdrop-blur-md text-white text-xs font-bold px-3 py-1.5 rounded-full flex items-center gap-1.5 shadow-lg">
                                    <AlertCircle size={12} /> Missed Trip
                                </div>
                            ) : trip.isOngoing ? (
                                <div className="bg-sky-500/90 backdrop-blur-md text-white text-xs font-bold px-3 py-1.5 rounded-full flex items-center gap-1.5 shadow-lg">
                                    <Plane size={12} className="animate-bounce" /> Trip Ongoing
                                </div>
                            ) : (
                                <div className="bg-sky-600/90 backdrop-blur-md text-white text-xs font-bold px-3 py-1.5 rounded-full flex items-center gap-1.5 shadow-lg">
                                    <Clock size={12} /> {trip.daysLeft != null ? (trip.daysLeft === 0 ? 'Starts Today!' : `${trip.daysLeft} days to go`) : 'Upcoming'}
                                </div>
                            )}
                        </>
                    )}
            ...
            <div className="flex flex-col flex-grow py-3 pr-4">
                <div className="flex justify-between items-start mb-2">
                    ...
                    <div className="relative group/menu">
                        ...
                        <div className="absolute right-0 top-full mt-1 bg-white border border-slate-100 shadow-xl rounded-xl py-2 w-32 hidden group-hover/menu:block z-30 animate-in fade-in slide-in-from-top-2 duration-200">
                            <button
                                onClick={(e) => { e.stopPropagation(); onClick(); }}
                                className="w-full text-left px-4 py-2 text-sm text-slate-600 hover:bg-slate-50 flex items-center gap-2 font-medium"
                            >
                            ...
                            {trip.status === 'upcoming' && !trip.isMissed && (
                                <button
                                    onClick={(e) => { e.stopPropagation(); onComplete(trip.id); }}
                ...
                <div className="flex flex-wrap items-center gap-3 mb-5">
                    <div className="flex items-center gap-2 bg-sky-50 text-sky-700 px-3 py-1.5 rounded-xl border border-sky-100 font-bold text-xs">
                        <Calendar size={14} />
                        <span>{trip.startDate && trip.startDate !== 'TBD' ? trip.startDate : 'TBD'} — {trip.endDate && trip.endDate !== 'TBD' ? trip.endDate : 'TBD'}</span>
                    </div>
                    {trip.startDate && trip.endDate && trip.startDate !== 'TBD' && (
                        <div className="bg-slate-100 text-slate-600 px-3 py-1.5 rounded-xl font-bold text-xs flex items-center gap-1.5">
                            <Clock size={14} />
                ...
                {trip.description && (
                    <div className="mb-6">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {['Adventure', 'Nature', 'Trekking', 'Photography'].slice(0, 3).map(tag => (
                                <span key={tag} className="px-2.5 py-1 bg-slate-50 text-slate-400 text-[10px] font-bold uppercase tracking-wider rounded-lg border border-slate-100">
                                    #{tag}
                                </span>
                            ))}
                        </div>
...
const TripDetailView = ({ trip, onBack, onDelete, onEdit, onComplete }: { trip: TripUI, onBack: () => void, onDelete: (id: string) => void, onEdit: (trip: TripUI) => void, onComplete: (id: string) => void }) => {
    const headerRef = useRef<HTMLDivElement>(null);
    const contentRef = useRef<HTMLDivElement>(null);

    const handleShare = () => {
        const text = `Check out my trip to ${trip.location}: ${trip.title}!`;
            ...
            <div ref={headerRef} className="relative h-72 md:h-96">
                ...
                <div className="absolute top-6 right-6 flex gap-3 z-20">
                    {trip.status === 'upcoming' && !trip.isMissed && (
                        <button
                            ...
                            className="px-6 py-3 bg-emerald-500 text-white rounded-full font-bold hover:bg-emerald-600 transition-all shadow-lg shadow-emerald-500/20 flex items-center gap-2"
                        >
                ...
                <div className="absolute bottom-8 left-8 right-8 text-white">
                    <div className="flex flex-wrap gap-3 mb-4">
                        <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider backdrop-blur-md border border-white/20 flex items-center gap-1.5 ${trip.status === 'upcoming' ? 'bg-sky-500/80' :
            ...
            <div ref={contentRef} className="p-6 md:p-10 bg-slate-50 min-h-[500px]">
                <div className="max-w-4xl mx-auto">
                    <div className="flex items-center justify-between mb-8">
                        ...
                        {trip.status === 'draft' && (
                            <button
                                onClick={() => alert("Activity planning coming soon!")}
                                className="flex items-center gap-2 px-4 py-2 bg-sky-600 text-white rounded-xl font-bold text-sm hover:bg-sky-700 transition-colors shadow-lg shadow-sky-600/20"
                            >
...
const TripsPage = ({ onLogout, onNavigate, isLoggedIn }: { onLogout: () => void, onNavigate: (page: string) => void, isLoggedIn?: boolean }) => {
    ...
    const handleCompleteTrip = async (id: string) => {
        try {
            ...
            const updated = await updateTrip(id, {
                tripName: tripToUpdate.title,
                placeName: tripToUpdate.location,
                tripDescription: tripToUpdate.description,
                startDate: tripToUpdate.startDate !== 'TBD' ? tripToUpdate.startDate : undefined,
                endDate: tripToUpdate.endDate !== 'TBD' ? tripToUpdate.endDate : undefined,
                status: 'PAST'
            });

            setTrips(prev => prev.map(t => t.id === id ? mapTripToUI(updated) : t));
            if (selectedTrip?.id === id) {
                setSelectedTrip(mapTripToUI(updated));
            }
            setActiveTab('past');
            alert("Congratulations on completing your adventure!");
        } catch (error) {
            const message = error instanceof Error ? error.message : 'Unable to complete trip.';
            setTripsError(message);
        }
    };

    const handleCreateTrip = async (e: React.FormEvent | null, statusOverride?: TripUIStatus) => {
        if (e) e.preventDefault();
        const effectiveStatus = statusOverride || (editingTripId ? (activeTab as any) : 'upcoming');

        // Basic validation for confirmed trips
        if (effectiveStatus === 'upcoming' && (!newTrip.startDate || !newTrip.endDate)) {
        ...
        try {
            if (editingTripId) {
                const updated = await updateTrip(editingTripId, {
                    tripName: newTrip.title,
                    placeName: newTrip.location,
                    tripDescription: `Your updated journey to ${newTrip.location}.`,
                    startDate: newTrip.startDate || undefined,
                    endDate: newTrip.endDate || undefined,
                    status: effectiveStatus === 'draft' ? 'DRAFT' : 'UPCOMING',
                    placePhotos: [matchingDest.image]
                });
                setTrips(prev => prev.map(t => t.id === editingTripId ? mapTripToUI(updated) : t));
                if (selectedTrip?.id === editingTripId) {
                    setSelectedTrip(mapTripToUI(updated));
                }
            } else {
                const created = await createTrip({
                    tripName: newTrip.title || "Untitled Trip",
                    placeName: newTrip.location || "Unknown Destination",
                    tripDescription: `Explore the wonders of ${newTrip.location}, a place where nature meets tradition. From breathtaking vistas to cultural landmarks, this journey is designed to inspire and rejuvenate your spirit. Discover why travelers from all over the world are drawn to the magic of this unique destination.`,
                    startDate: newTrip.startDate || undefined,
                    endDate: newTrip.endDate || undefined,
                    status: effectiveStatus === 'draft' ? 'DRAFT' : 'UPCOMING',
                    placePhotos: [matchingDest.image]
            ...
            setActiveTab(effectiveStatus === 'draft' ? 'draft' : 'upcoming');
            setTripsError(null);
        } catch (error) {
    ...
    const handleOpenEdit = (trip: TripUI) => {
        setEditingTripId(trip.id);
        setNewTrip({
            title: trip.title,
            location: trip.location,
            startDate: trip.startDate !== 'TBD' ? trip.startDate : '',
            endDate: trip.endDate !== 'TBD' ? trip.endDate : ''
        });
        setIsModalOpen(true);
    };
                ...
                <div className="hidden md:flex items-center gap-2 bg-white/80 backdrop-blur-xl px-2 py-2 rounded-full border border-white/50 shadow-sm ring-1 ring-slate-100">
                    {navItems.map((item, i) => (
                        <button
                            ...
                            className={`px-6 py-2.5 rounded-full text-sm font-bold transition-all duration-300 flex items-center gap-2 ${item.page === 'trips'
                        ...
                        <div className="trips-header-anim border-b border-slate-200 mb-8 flex gap-8">
                            {[
                                ...
                                <button
                                    ...
                                    className={`pb-4 text-sm font-bold uppercase tracking-wider transition-all relative ${activeTab === tab.id
                                        ? 'text-sky-600'
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
                    ...
                    >
                        ...
                        <h2 className="text-2xl font-bold font-display text-slate-900 mb-1">
                            {editingTripId ? 'Edit Adventure' : 'Plan New Adventure'}
                        </h2>
                        <p className="text-slate-500 text-sm mb-6">
                            {editingTripId ? 'Update your trip details' : 'Where are you heading next?'}
                        </p>
