   ...
   useEffect(() => {
      const loadProfile = async () => {
         ...
         try {
            ...
            setUserProfile(prev => ({
               ...prev,
               name: profile.name || prev.name || 'Explorer',
               email: profile.email || prev.email,
               phone: profile.phoneNumber || prev.phone,
               location: profile.location || prev.location,
               title: profile.title || prev.title || 'Explorer',
   ...
   const handleSaveProfile = async () => {
      ...
      try {
         ...
         setUserProfile(prev => ({
            ...
            budget: updated.budget !== undefined ? updated.budget : prev.budget,
            travelStyle: updated.travelStyle || prev.travelStyle,
         }));

         setIsEditing(false);
         gsap.to('.save-feedback', {
            opacity: 1, y: -10, duration: 0.5, onComplete: () => {
            ...
            <div className="hidden md:flex items-center gap-2 bg-white/80 backdrop-blur-xl px-2 py-2 rounded-full border border-white/50 shadow-sm ring-1 ring-slate-100">
               {navItems.map((item, i) => (
                  <button
                     ...
                     className={`px-6 py-2.5 rounded-full text-sm font-bold transition-all duration-300 flex items-center gap-2 ${item.page === 'profile'
                           ...
                           <>
                              <h1 className="text-3xl font-display font-bold text-slate-900 overflow-hidden">
                                 {isLoadingProfile ? (
                                    ...
                                    splitText(userProfile.name || "Explorer")
                                 )}
                              </h1>
                        ...
                        <div className="flex flex-wrap gap-2">
                           {AVAILABLE_INTERESTS.map((interest) => (
                              <button
                                 ...
                                 disabled={!isEditing}
                                 className={`px-4 py-2 rounded-xl text-sm font-bold transition-all ${userProfile.interests.includes(interest)
