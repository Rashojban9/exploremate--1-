...
// High-Fidelity GSAP Decoding Text Component
...
const LoginPage = ({ onLogin, onNavigate }: { onLogin: () => void, onNavigate: (page: string) => void }) => {
  ...
  useEffect(() => {
    ...
    gsap.fromTo('.form-element',
      { x: -20, opacity: 0 },
      { x: 0, opacity: 1, stagger: 0.08, duration: 0.8, ease: "power4.out", delay: 0.6 }
    );
  }, []);
        ...
        <div className="w-full md:w-[45%] p-8 sm:p-12 lg:p-20 flex flex-col justify-center bg-white relative">
          ...
          <div className="mb-14">
            ...
            <DecodingHeader 
              ...
              className="text-3xl sm:text-4xl lg:text-5xl font-display font-extrabold text-slate-900 mb-6 leading-[1.05] tracking-tight min-h-[6rem] sm:min-h-[7rem]" 
            />
          ...
          <form ref={formRef} onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="form-element flex items-center justify-between text-[11px] font-bold pt-4 text-slate-400">
              <button type="button" onClick={() => onNavigate('signup')} className="hover:text-sky-600 transition-colors uppercase tracking-wider">Create Account</button>
              <button type="button" onClick={() => onNavigate('forgot-password')} className="hover:text-sky-600 transition-colors uppercase tracking-wider">Forgot Password</button>
            </div>
          </form>
        </div>
          ...
          {LOGIN_SLIDES.map((slide, index) => (
            ...
            >
              <img 
                src={slide.image} 
                alt={slide.title} 
                className={`w-full h-full object-cover opacity-60 transition-transform duration-[15000ms] ease-linear ${
                   index === currentSlide ? 'scale-110 translate-x-4' : 'scale-100'
                }`}
                onError={(e) => {
            ...
            {LOGIN_SLIDES.map((slide, index) => (
               index === currentSlide && (
                <div key={slide.id} className="animate-in fade-in slide-in-from-bottom-12 duration-1200">
                  ...
                  <h2 className="text-4xl lg:text-6xl font-display font-bold text-white mb-6 leading-[1.1] tracking-tight">
                    {slide.title}
                  </h2>
                  <p className="text-slate-300 text-lg lg:text-xl max-w-md leading-relaxed opacity-80 font-medium">
                    {slide.description}
                  </p>
                </div>
               )
              ...
              {LOGIN_SLIDES.map((_, i) => (
                <button
                  ...
                  className={`h-1 rounded-full transition-all duration-700 ${
                    i === currentSlide ? 'w-16 bg-white shadow-[0_0_15px_rgba(255,255,255,0.5)]' : 'w-4 bg-white/20 hover:bg-white/40'
                  }`}
                />
              ))}
            </div>
